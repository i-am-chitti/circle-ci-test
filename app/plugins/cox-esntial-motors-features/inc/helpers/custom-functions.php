<?php
/**
 * Cox_Esntial_Motors features custom functions.
 *
 ...
 */
function cox_esntial_motors_get_cache_key( $unique = '' ) {

	$cache_key = 'cox_esntial_motors_cache_';
 ...
 */
function cox_esntial_motors_get_cached_posts( $args ) {
	...
	$cache_key = cox_esntial_motors_get_cache_key( $args );

	$cache  = new \Cox_Esntial_Motors\Features\Inc\Cache( $cache_key );
 ...
 */
function cox_esntial_motors_get_template_content( $slug, $vars = [] ) {
 ...
 */
function cox_esntial_motors_features_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', COX_ESNTIAL_MOTORS_FEATURES_PATH, $template );
 ...
 */
function cox_esntial_motors_get_data( $slug, $default = [] ) {

	$data_file = sprintf( COX_ESNTIAL_MOTORS_FEATURES_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function cox_esntial_motors_is_production() {
 ...
 */
function cox_esntial_motors_is_mobile( $kind = 'any', $return_matched_agent = false ) {
