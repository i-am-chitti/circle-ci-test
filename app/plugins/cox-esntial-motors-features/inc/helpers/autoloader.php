 ...
 *
 * @package cox-esntial-motors-featuers
 */

namespace Cox_Esntial_Motors\Features\Inc\Helpers;
...
function autoloader( $resource = '' ) {
	...
	$resource_path  = false;
	$namespace_root = 'Cox_Esntial_Motors\Features\\';
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( COX_ESNTIAL_MOTORS_FEATURES_PATH ), $directory, $file_name );
	...
	if ( 'tests' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/tests/php/%s.php', untrailingslashit( COX_ESNTIAL_MOTORS_FEATURES_PATH ), $file_name );
...
spl_autoload_register( '\Cox_Esntial_Motors\Features\Inc\Helpers\autoloader' );
